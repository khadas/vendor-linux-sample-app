...
display_help() {
	...
	***************************HELP**************************
    ...
    EXAMPLES:
    *.sh media0 1920 1080 RGB 30/1 dmabuf 100 amlfd
    *.sh 
	"
}
...
CMD="gst-launch-1.0 -v v4l2src device=/dev/$DEVICE $USE_IO_MODE $USE_FRAME_CNT ! video/x-raw,width=$SRC_WIDTH,height=$SRC_HEIGHT,format=$SRC_FORMAT,framerate=$SRC_FRAMERATE ! amlnn detection-model=$DETECT_MODE ! amlnnoverlay ! $SINK"
echo $CMD
